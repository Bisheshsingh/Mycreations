...
def numberEnter(num):
        global result,ip_val,current,val
        result=False
...
def operation(op):
        global check_sum,ip_val,total,current,result,op1,val
        if op=='add':
            val+='+'
        elif op=='sub':
            val+='-'
...
def clear_entry():
        global current,result,ip_val,val
        result=False
        if len(a.get().strip()):
           val=a.get()[:len(a.get().strip())-1]
        a.set(val)
        current='0'
        display(0)
        ip_val=True
...
a=StringVar()
Entry(calc,textvar=a).place(x=10,y=10)
#Display control
menubar=Menu(calc)
